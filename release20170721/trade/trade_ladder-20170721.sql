...
CREATE TABLE `ladder_account_transaction` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='账户变动记录表';
...
CREATE TABLE `ladder_batch` (
  ...
  `trade_biz_code` varchar(32) DEFAULT NULL COMMENT '022-申购申请 122-申购确认 024-赎回 025-预约赎回 124-到期兑付确认 125预约赎回确认 201-每日收益 301-还款计划',
  ...
  `original_batchNo` varchar(64) DEFAULT NULL COMMENT '原批次号',
  `sale_channel` varchar(32) DEFAULT NULL COMMENT '渠道:TXS-唐小僧 QYLC-企业理财 YW-摇旺',
  `expect_endliquidation_date` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '预计结清日',
  `confirm_count` int(11) DEFAULT '0' COMMENT '已确认条数',
  PRIMARY KEY (`id`),
  ...
  KEY `tbc_pd_s_ct` (`trade_biz_code`,`product_code`,`status`,`create_time`),
  FULLTEXT KEY `productCode_index` (`product_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='批次记录表';
...
CREATE TABLE `ladder_contract` (
  ...
  `assignee_cooperation_agency_code` varchar(64) DEFAULT NULL COMMENT '受托方合作机构代码',
  `cooperation_agency` varchar(64) DEFAULT NULL COMMENT '定向投资标的合作机构(发行方名称)',
  `invest_amount` decimal(16,4) DEFAULT '0.0000' COMMENT '投资金额',
  `year_yield` decimal(16,4) DEFAULT '0.0000' COMMENT '年化收益率',
  `invest_date` datetime DEFAULT NULL COMMENT '委托投资日(用户投资成功日期，精确到年月日)',
  `expire_date` datetime DEFAULT NULL COMMENT '委托到期日(此阶梯产品到期日)',
  `withdrawal_commission_json` varchar(5120) DEFAULT NULL COMMENT '阶段提前退出手续费扣减json',
  `assets_match_json` varchar(5120) DEFAULT NULL COMMENT '订单对应资产匹配json',
  `status` varchar(32) DEFAULT 'INIT' COMMENT 'INIT-初始化 ; FAIL-废弃',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  `create_by` varchar(64) DEFAULT NULL,
  `modify_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `modify_by` varchar(64) DEFAULT NULL,
  ...
  `product_name` varchar(64) DEFAULT NULL COMMENT '产品名称',
  PRIMARY KEY (`id`),
  UNIQUE KEY `order_salechannel` (`order_no`,`sale_channel`)
...
CREATE TABLE `ladder_daily_income` (
  ...
  `status` varchar(32) DEFAULT NULL COMMENT '状态:INIT ; PROCESSING:处理中; SUCCESS处理完成 ;',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  `create_by` varchar(64) DEFAULT NULL,
  `modify_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `modify_by` varchar(64) DEFAULT NULL,
  ...
  KEY `pId_pc_incomeD_status` (`platform_member_id`,`product_code`,`income_date`,`status`),
  FULLTEXT KEY `originTradeSerialNo_index` (`origin_trade_serial_no`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='每日收益记录表';
...
CREATE TABLE `ladder_error_log` (
  ...
  `trace` varchar(1024) DEFAULT NULL,
  PRIMARY KEY (`id`)
...
CREATE TABLE `ladder_invest_confirm` (
  ...
  `sale_channel` varchar(32) DEFAULT NULL COMMENT '渠道:TXS-唐小僧 QYLC-企业理财 YW-摇旺',
  `status` varchar(32) DEFAULT NULL COMMENT '状态-INIT 处理中-PROCESSING 成功-COMPLETED 失败-FAIL',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  `create_by` varchar(64) DEFAULT NULL,
  `modify_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `modify_by` varchar(64) DEFAULT NULL,
  `contract_status` varchar(32) DEFAULT 'INIT' COMMENT '合同状态',
  `memo` varchar(1024) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`),
  ...
  KEY `status_contractStatus` (`status`,`contract_status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='投资确认记录表';
...
CREATE TABLE `ladder_invest_request` (
  ...
  `group_serial_no` varchar(64) DEFAULT NULL,
  ...
  `order_no` varchar(64) DEFAULT NULL COMMENT '订单系统-order_no',
  `product_code` varchar(64) DEFAULT NULL COMMENT '产品编码',
  `product_name` varchar(64) DEFAULT NULL COMMENT '产品名称',
  `invest_amount` decimal(16,4) DEFAULT '0.0000' COMMENT '投资金额',
  ...
  `status` varchar(32) DEFAULT NULL COMMENT '状态:初始化INIT;生成文件处理完成 COMPLETED;',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  `create_by` varchar(64) DEFAULT NULL,
  `modify_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `modify_by` varchar(64) DEFAULT NULL,
  ...
  KEY `platformMemberId_index` (`platform_member_id`),
  KEY `productCode_index` (`product_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='投资申请记录表';
...
CREATE TABLE `ladder_redeem_confirm` (
  ...
  `invest_amount` decimal(16,4) DEFAULT '0.0000' COMMENT '投资金额(本金)',
  `interest_amount` decimal(16,4) DEFAULT '0.0000' COMMENT '计息金额',
  `redeem_income` decimal(16,4) DEFAULT '0.0000' COMMENT '兑付收益',
  `redeem_total_amount` decimal(16,4) DEFAULT '0.0000' COMMENT '赎回金额',
  ...
  `interest_days` int(32) DEFAULT '0' COMMENT '计息天数',
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='赎回确认记录表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='赎回申请记录表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='交易系统序列表';
...
CREATE TABLE `ladder_stage_income_plan` (
  ...
  `sale_channel` varchar(32) DEFAULT NULL COMMENT '渠道:TXS-唐小僧 QYLC-企业理财 YW-摇旺',
  PRIMARY KEY (`id`),
  UNIQUE KEY `appSheetNo_currentStageNum_index` (`app_sheet_no`,`current_stage_num`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='还款计划记录表';

...
CREATE TABLE `asset_file` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8 COMMENT='上传文件';
...
CREATE TABLE `template` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键',
  `template_type` tinyint(2) DEFAULT NULL COMMENT '模板类型 1：产品注册模板 2：签章模板',
  ...
  `modify_by` varchar(32) DEFAULT 'SYS' COMMENT '最后更新者',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=utf8 COMMENT='模板配置表';
...
CREATE TABLE `user` (
  ...
  `login_type` tinyint(1) DEFAULT NULL COMMENT '登录类型（1：用户名登录， 2：域账户统一登录）',
  PRIMARY KEY (`id`)

...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`neptune` /*!40100 DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci */;
...
CREATE TABLE `account` (
  ...
  `sale_channel` varchar(32) NOT NULL COMMENT '渠道 QJS-侨金所',
  `product_code` varchar(32) NOT NULL COMMENT '产品代码',
  `total_amount` decimal(16,4) NOT NULL DEFAULT '0.0000' COMMENT '累计总本金',
  `total_income` decimal(16,4) NOT NULL DEFAULT '0.0000' COMMENT '累计总收益',
  ...
  `modify_by` varchar(32) NOT NULL DEFAULT 'system' COMMENT '修改人',
  PRIMARY KEY (`id`),
  UNIQUE KEY `mIdsChannelPcode` (`member_id`,`sale_channel`,`product_code`)
) ENGINE=InnoDB AUTO_INCREMENT=14047 DEFAULT CHARSET=utf8 COMMENT='账户表';
...
CREATE TABLE `batch` (
  ...
  `type` varchar(16) NOT NULL COMMENT '//产品流标FAILURE,//匹配MATCH,//放款LOAN,//合同CONTRACT,//兑付计划CASHPLAN,//每日收益INCOME,//兑付CASH//逾期兑付OVERDUECASH',
  `status` varchar(32) NOT NULL COMMENT 'INIT PROCESSING COMPLETE FAIL',
  `product_code` varchar(64) NOT NULL COMMENT '产品编号',
  `version` int(11) NOT NULL DEFAULT '1',
  `memo` varchar(218) DEFAULT '' COMMENT '备注',
  `create_by` varchar(64) NOT NULL DEFAULT 'system',
  `create_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `modify_by` varchar(64) NOT NULL DEFAULT 'system',
  `modify_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `sale_channel` varchar(32) NOT NULL COMMENT '渠道 QJS 侨金所',
  `total_amount` decimal(16,4) DEFAULT '0.0000',
  `set_up_type` varchar(32) DEFAULT '' COMMENT '自动成立：AUTO_SETUP,部分成立：PART_SETUP',
...
) ENGINE=InnoDB AUTO_INCREMENT=732 DEFAULT CHARSET=utf8 COMMENT='批次表';
...
CREATE TABLE `cash_record` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键id',
  `app_sheet_no` varchar(64) NOT NULL COMMENT '投资申请号',
  `ext_order_no` varchar(64) NOT NULL COMMENT '外部系统订单号',
  `product_code` varchar(64) NOT NULL COMMENT '产品代码',
  `member_id` varchar(64) NOT NULL COMMENT '平台用户ID',
  `sale_channel` varchar(8) NOT NULL COMMENT '渠道 唐小僧 TXS|企业理财QYLIC|摇旺YW',
  `amount` decimal(16,4) NOT NULL DEFAULT '0.0000' COMMENT '本金',
  `income` decimal(16,4) NOT NULL DEFAULT '0.0000' COMMENT '收益',
  `cash_amount` decimal(16,4) NOT NULL DEFAULT '0.0000' COMMENT '兑付总金额',
  ...
  `sign_id` varchar(64) NOT NULL COMMENT '投资人签约协议号',
  `cash_ref_no` varchar(64) NOT NULL DEFAULT '' COMMENT '兑付流水号',
  `memo` varchar(256) NOT NULL DEFAULT '',
  `status` varchar(16) NOT NULL COMMENT 'INIT:待处理；CASHING:兑付中；CASH_SUCCESS:兑付成功；CASH_FAIL:兑付失败；OVERDUE:逾期',
  `version` int(11) NOT NULL DEFAULT '1',
  `create_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
) ENGINE=InnoDB AUTO_INCREMENT=37677 DEFAULT CHARSET=utf8 COMMENT='投资人兑付表';
...
CREATE TABLE `contract` (
  ...
  `create_time` timestamp(3) NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '生成时间',
...
) ENGINE=InnoDB AUTO_INCREMENT=54274 DEFAULT CHARSET=utf8;
...
CREATE TABLE `daily_income` (
  ...
  `account_id` varchar(64) NOT NULL DEFAULT '' COMMENT '交易账号',
  `product_code` varchar(64) NOT NULL DEFAULT '' COMMENT '产品code',
  `app_sheet_no` varchar(64) NOT NULL DEFAULT '' COMMENT 'TA投资记录流水号',
  ...
  `ext_order_no` varchar(64) NOT NULL DEFAULT '' COMMENT '外部订单号',
  `member_id` varchar(64) NOT NULL DEFAULT '',
  `sale_channel` varchar(8) NOT NULL DEFAULT '' COMMENT '渠道 |侨金所QJS',
  `create_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `create_by` varchar(64) NOT NULL DEFAULT 'system',
  `modify_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `modify_by` varchar(64) NOT NULL DEFAULT 'system',
...
) ENGINE=InnoDB AUTO_INCREMENT=1807810 DEFAULT CHARSET=utf8;
...
CREATE TABLE `invest` (
  ...
  `app_sheet_no` varchar(64) NOT NULL COMMENT '交易系统流水号',
  `ext_order_no` varchar(64) NOT NULL COMMENT '外部系统订单号',
  `member_id` varchar(64) NOT NULL COMMENT '会员ID',
  `member_name` varchar(64) NOT NULL COMMENT '姓名',
  `cert_no` varchar(32) NOT NULL COMMENT '证件号',
  `product_code` varchar(64) NOT NULL COMMENT '产品编码',
  `product_name` varchar(64) NOT NULL COMMENT '产品名称',
  `invest_amount` decimal(16,4) NOT NULL DEFAULT '0.0000' COMMENT '投资金额',
  `sale_channel` varchar(32) NOT NULL COMMENT '渠道:TXS-唐小僧',
  `order_time` datetime DEFAULT NULL COMMENT '下单时间',
  `status` varchar(32) NOT NULL DEFAULT 'MATCH_INIT' COMMENT '状态:MATCH_INIT-匹配中;MATCH_SUCCESS-匹配成功;MATCH_FAIL-流标',
  `cash_flag` varchar(8) NOT NULL DEFAULT 'FALSE' COMMENT '兑付标志：FALSE-未生成兑付计划；TRUE-已生成兑付计划;',
  `interest_flag` varchar(8) NOT NULL DEFAULT 'FALSE' COMMENT '收益计算标志,FALSE:未生成,TRUE:已生成',
  `version` int(11) NOT NULL DEFAULT '1',
  `create_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `create_by` varchar(64) NOT NULL DEFAULT 'system',
  `modify_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  `modify_by` varchar(64) NOT NULL DEFAULT 'system',
  `investor_bank_account_name` varchar(64) NOT NULL COMMENT '投资人银行账户户名',
  `investor_opening_bank` varchar(128) NOT NULL COMMENT '投资人银行账户开户行',
  `investor_bank_card_no` varchar(64) NOT NULL COMMENT '投资人银行账号',
  `sign_id` varchar(64) NOT NULL COMMENT '投资人签约协议号',
  `match_flag` varchar(8) NOT NULL DEFAULT 'FALSE' COMMENT '匹配标志:FALSE-未生成；TRUE-已生成;',
  `loan_flag` varchar(8) NOT NULL DEFAULT 'FALSE' COMMENT '放款标志:FALSE-未生成；TRUE-已生成;',
  `mobile_no` varchar(32) DEFAULT '' COMMENT '手机号',
  PRIMARY KEY (`id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=54647 DEFAULT CHARSET=utf8 COMMENT='投资表';
...
) ENGINE=InnoDB AUTO_INCREMENT=1486 DEFAULT CHARSET=utf8 COMMENT='接口请求重试表';
...
CREATE TABLE `loan_record` (
  ...
  `sale_channel` varchar(8) NOT NULL COMMENT '渠道 唐小僧 TXS|企业理财QYLIC|摇旺YW',
  `amount` decimal(16,2) NOT NULL DEFAULT '0.00' COMMENT '本金',
  ...
  `status` varchar(16) NOT NULL DEFAULT 'LOAN_PROCESSING' COMMENT 'LOAN_PROCESSING:放款中LOAN_SUCCESS:放款成功',
  `version` int(11) NOT NULL DEFAULT '1',
  `create_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `create_by` varchar(32) NOT NULL DEFAULT 'system',
  `modify_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  ...
  `loan_no` varchar(64) NOT NULL COMMENT '放款流水号',
  PRIMARY KEY (`id`),
  UNIQUE KEY `appsheetnoIndex` (`app_sheet_no`),
  UNIQUE KEY `payNoIndex` (`pay_no`),
  KEY `lonaIndex` (`loan_no`)
) ENGINE=InnoDB AUTO_INCREMENT=37744 DEFAULT CHARSET=utf8 COMMENT='投资人放款表';
...
CREATE TABLE `match_record` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `member_id` varchar(64) NOT NULL COMMENT '平台用户ID',
  `asset_code` varchar(64) NOT NULL COMMENT '资产code',
  `asset_amount` decimal(16,4) DEFAULT '0.0000' COMMENT '资产总金额',
  `app_sheet_no` varchar(64) NOT NULL COMMENT '交易系统生成的订单编号',
  `ext_order_no` varchar(64) NOT NULL COMMENT '外部订单号（唐小僧）',
  `product_code` varchar(64) NOT NULL COMMENT '产品code',
  `matched_amount` decimal(16,4) NOT NULL DEFAULT '0.0000' COMMENT '匹配金额',
  `match_status` varchar(32) NOT NULL DEFAULT 'MATCH_SUCCESS' COMMENT '匹配状态(MATCH_SUCCESS:匹配成功',
  `version` int(11) NOT NULL DEFAULT '1' COMMENT '版本号',
  `create_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  `modify_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '修改时间',
  `create_by` varchar(32) NOT NULL DEFAULT 'system' COMMENT '创建人',
  `modify_by` varchar(32) NOT NULL DEFAULT 'system' COMMENT '修改人',
  `financing_member_Id` varchar(64) NOT NULL DEFAULT '' COMMENT '融资机构会员id',
  `financing_code` varchar(64) NOT NULL DEFAULT '' COMMENT '融资机构编码',
  PRIMARY KEY (`id`),
  UNIQUE KEY `appsheetnoIndex` (`app_sheet_no`)
) ENGINE=InnoDB AUTO_INCREMENT=37895 DEFAULT CHARSET=utf8 COMMENT='配订单表';
...
CREATE TABLE `product` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键id',
  `sale_channel` varchar(8) NOT NULL COMMENT '渠道',
  `product_code` varchar(64) NOT NULL COMMENT '产品代码',
  `product_name` varchar(128) NOT NULL COMMENT '产品名称',
  `expect_clear_date` date DEFAULT NULL COMMENT '回款日',
  `status` varchar(16) NOT NULL DEFAULT 'INIT' COMMENT '状态 INIT:初始SETUP:产品成立;FAILURE:流标;',
  `amount` decimal(16,4) DEFAULT '0.0000' COMMENT '本金',
  `count` int(11) DEFAULT '0' COMMENT '投资笔数',
  `asset_code` varchar(64) DEFAULT '' COMMENT '资产编号',
  `financing_member_id` varchar(64) DEFAULT '' COMMENT '融资方会员ID',
  `financing_party_name` varchar(128) DEFAULT '' COMMENT '融资方名称',
  `financing_code` varchar(64) DEFAULT '' COMMENT '融资方编码',
...
) ENGINE=InnoDB AUTO_INCREMENT=176 DEFAULT CHARSET=utf8 COMMENT='产品';
